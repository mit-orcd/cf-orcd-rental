...
# =============================================================================
# Install upstream-migration copies into the cloned ColdFront tree
# =============================================================================
# ColdFront v1.1.7 ships with model/migration drift in allocation, grant,
# publication, research_output, and resource. Without the upstream-migration
# copies kept here, `migrate` and `initial_setup` print:
#   "Your models in app(s): ... have changes that are not yet reflected in a
#    migration, and so won't be applied."
# These files were generated by `manage.py makemigrations` against a clean
# v1.1.7 checkout with deps from upstream uv.lock. See
# tests/setup/upstream_migration_copies/README.md for regeneration steps.

log_step "Installing upstream ColdFront core migration copies"

UPSTREAM_MIGRATIONS_DIR="$SCRIPT_DIR/upstream_migration_copies"
if [ -d "$UPSTREAM_MIGRATIONS_DIR" ]; then
    for app in allocation grant publication research_output resource; do
        src_dir="$UPSTREAM_MIGRATIONS_DIR/$app"
        dst_dir="$COLDFRONT_DIR/coldfront/core/$app/migrations"
        if [ -d "$src_dir" ] && [ -d "$dst_dir" ]; then
            # Copy every upstream-migration *.py file (skip README, etc.)
            shopt -s nullglob
            files=("$src_dir"/*.py)
            shopt -u nullglob
            if [ ${#files[@]} -gt 0 ]; then
                cp "${files[@]}" "$dst_dir/"
                names=$(printf '%s ' "${files[@]##*/}")
                echo "  $app: installed ${names}"
            fi
        fi
    done
else
    log_warn "Upstream migration copies directory not found at $UPSTREAM_MIGRATIONS_DIR"
fi

# =============================================================================
# Apply Migrations
# =============================================================================
# After this step there should be NO "changes that are not yet reflected in a
# migration" warning. If one appears, ColdFront's models have drifted further
# upstream and the upstream_migration_copies/ directory needs to be regenerated
# (see its README.md).
...
# Safety net: warn (don't fail) if upstream ColdFront has drifted further
# than the upstream-migration copies cover.
if [ "$INSTALLER" = "uv" ]; then
    uv run coldfront makemigrations --check --dry-run >/dev/null 2>&1 \
        || log_warn "ColdFront has new model drift; regenerate tests/setup/upstream_migration_copies/."
else
    coldfront makemigrations --check --dry-run >/dev/null 2>&1 \
        || log_warn "ColdFront has new model drift; regenerate tests/setup/upstream_migration_copies/."
fi
